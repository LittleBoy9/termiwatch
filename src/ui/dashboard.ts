...
export class Dashboard {
  ...
  private renderHeader(metrics: Metrics, w: number): string {
    const title = `${color.bold}${color.brightCyan} ⚡ termiwatch${color.reset}`;
  ...
  private renderFooter(w: number): string {
    const help = `${color.dim} Tab${color.reset}${color.gray} switch view${color.reset}  ${color.dim}q${color.reset}${color.gray} exit${color.reset}  ${color.dim}termiwatch v1.0.0${color.reset}`;
